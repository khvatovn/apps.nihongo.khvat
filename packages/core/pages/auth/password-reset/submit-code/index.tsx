...
import { resendResetCode, verifyResetCode } from "@nihongo/core/features/auth/api";
...
type SubmitCodeNavProp = StackNavigationProp<
  AuthParamList,
  typeof AUTH_ROUTES.RESET_PASSWORD_SUBMIT_CODE
>;
type SubmitCodeRouteProp = RouteProp<AuthParamList, typeof AUTH_ROUTES.RESET_PASSWORD_SUBMIT_CODE>;

export const ResetPasswordSubmitCodePage: React.FC = () => {
  ...
  const verify = async () => {
    ...
    setSubmitting(true);
    try {
      const { ok, body } = await verifyResetCode(email, code, region);

      if (ok && body.reset_token) {
        navigation.navigate(AUTH_ROUTES.RESET_PASSWORD_CONFIRM, {
          email,
          region,
  ...
  const resend = async () => {
    ...
    setError(undefined);
    try {
      const { ok, status, body } = await resendResetCode(email, region);
